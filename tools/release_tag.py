#!/usr/bin/env python3
"""Create a TradeTide release commit and annotated semantic-version tag."""
...
def main() -> int:
    ...
    parser.add_argument("tag", help="annotated release tag, for example v0.3.1")
    arguments = parser.parse_args()
    match = TAG_PATTERN.fullmatch(arguments.tag)
    if match is None:
        print("release aborted: tag must use vMAJOR.MINOR.PATCH, for example v0.3.1", file=sys.stderr)
        return 1
    try:
        ...
        run("git", "add", "TradeTide/_version.py")
