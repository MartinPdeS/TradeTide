    ...
    public:
        ...
        void update_price(BasePosition& position, const size_t time_idx, const double &current_price) {
            ...
            if (this->save_price_data) {
                this->dates.push_back(position.prices->dates[time_idx]);
...
class StaticExitStrategy : public ExitStrategy {
    public:
        ...
        void update_stop_loss_price(BasePosition& position, const double&) override {
            if (position.is_long)
                this->stop_loss_price = position.entry_price - this->stop_loss_pip * this->pip_value;
            else
                this->stop_loss_price = position.entry_price + this->stop_loss_pip * this->pip_value;
        }
        ...
        void update_take_profit_price(BasePosition& position, const double&) override {
            if (position.is_long)
                this->take_profit_price = position.entry_price + this->take_profit_pip * this->pip_value;
            else
                this->take_profit_price = position.entry_price - this->take_profit_pip * this->pip_value;
        }
    };
...
class TrailingExitStrategy : public ExitStrategy {
    ...
    public:
        ...
        void update_stop_loss_price(BasePosition& position, const double& current_price) override {
            if (position.is_long) {
                double current_stop_loss_price = current_price - this->stop_loss_pip * this->pip_value;
        ...
        void update_take_profit_price(BasePosition& position, const double& current_price) override {
            if (position.is_long) {
                double current_take_profit_price = current_price + this->take_profit_pip * this->pip_value;
    ...
    public:
        ...
        void update_stop_loss_price(BasePosition& position, const double& current_price) override {
            if (!break_even_triggered) {
                ...
                if (position.is_long)
                    stop_loss_price = position.entry_price - stop_loss_pip * this->pip_value;
                else
                    stop_loss_price = position.entry_price + stop_loss_pip * this->pip_value;

                double distance_moved = std::abs(current_price - position.entry_price) / this->pip_value;
        ...
        void update_take_profit_price(BasePosition& position, const double&) override {
            if (position.is_long)
                take_profit_price = position.entry_price + take_profit_pip * this->pip_value;
            else
                take_profit_price = position.entry_price - take_profit_pip * this->pip_value;
        }
    };
