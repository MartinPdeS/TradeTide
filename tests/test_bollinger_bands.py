...
def test_initialization_with_timedelta_window(indicator):
    ...
    """
    assert (
        indicator.window == DEFAULT_WINDOW
    ), f"Expected window {DEFAULT_WINDOW}, got {indicator.window}"
    assert (
        indicator.multiplier == DEFAULT_MULTIPLIER
    ), f"Expected multiplier {DEFAULT_MULTIPLIER}, got {indicator.multiplier}"
...
@pytest.mark.parametrize(
    "window,multiplier",
    [(10 * minutes, 1.5), (20 * minutes, 2.0), (1 * hours, 2.5), (1 * days, 3.0)],
)
...
def test_bollinger_bands_array_lengths(sample_market):
    ...
    assert (
        len(indicator._cpp_sma) == market_length
    ), f"SMA length {len(indicator._cpp_sma)} should match market length {market_length}"
    assert (
        len(indicator._cpp_upper_band) == market_length
    ), f"Upper band length {len(indicator._cpp_upper_band)} should match market length {market_length}"
    assert (
        len(indicator._cpp_lower_band) == market_length
    ), f"Lower band length {len(indicator._cpp_lower_band)} should match market length {market_length}"
    assert (
...
def test_bollinger_bands_mathematical_properties(sample_market):
    ...
    if len(sma_valid) > 0:
        ...
        np.testing.assert_allclose(
            ...
            lower_distance,
            rtol=1e-10,
            err_msg="Bands should be symmetric around SMA",
        )
...
def test_band_expansion_in_volatile_market(volatile_prices):
    ...
    if np.sum(valid_mask) > 10:  # Need sufficient data points
        ...
        assert np.all(
            band_width > 0
        ), "Band width should be positive in volatile markets"
...
def test_band_contraction_in_sideways_market(sideways_prices):
    ...
    if np.sum(valid_mask) > 10:
        ...
        band_width_cv = np.std(band_width) / np.mean(
            band_width
        )  # Coefficient of variation
        assert (
            band_width_cv < 1.0
        ), "Band width should be relatively stable in sideways markets"
...
def test_edge_case_insufficient_data():
    ...
    try:
        ...
        assert len(indicator._cpp_sma) == len(
            short_dates
...
def test_different_multiplier_values(sample_market, multiplier):
    ...
    if np.sum(valid_mask) > 0:
        ...
        assert (
            band_width > 0
        ), f"Band width should be positive for multiplier {multiplier}"
...
def test_different_market_data_frequencies(frequency):
    ...
    assert len(indicator._cpp_upper_band) == len(
        dates
    ), f"Output length mismatch for {frequency}min frequency"
    assert len(indicator._cpp_lower_band) == len(
        dates
    ), f"Output length mismatch for {frequency}min frequency"
...
def test_numerical_stability_extreme_values():
    ...
    for prices, description in extreme_test_cases:
        ...
        if np.sum(valid_mask) > 0:
            assert not np.any(
                np.isinf(sma[valid_mask])
            ), f"SMA should not contain infinite values for {description}"
            assert not np.any(
                np.isinf(upper_band[valid_mask])
            ), f"Upper band should not contain infinite values for {description}"
            assert not np.any(
                np.isinf(lower_band[valid_mask])
            ), f"Lower band should not contain infinite values for {description}"
...
def test_end_to_end_workflow(sample_market):
    ...
    assert (
        len(indicator._cpp_upper_band) == data_length
    ), "Data length consistency check"
    assert (
        len(indicator._cpp_lower_band) == data_length
    ), "Data length consistency check"
    ...
    if np.sum(valid_mask) > 0:
        assert np.all(
            upper_band[valid_mask] >= sma[valid_mask]
        ), "Upper band should be >= SMA"
        assert np.all(
            lower_band[valid_mask] <= sma[valid_mask]
        ), "Lower band should be <= SMA"
        assert np.all(
            sma[valid_mask] > 0
        ), "SMA should be positive for positive price data"
