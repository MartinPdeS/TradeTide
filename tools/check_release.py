#!/usr/bin/env python3
"""Check that TradeTide release metadata agrees across project files."""
...
def versions() -> dict[str, str]:
    ...
    return {
        ...
        "conda.recipe/meta.yaml": extract(r"^\s*version:\s*['\"]?([^'\"\s]+)", CONDA_RECIPE_PATH, "Conda version"),
        "TradeTide/_version.py": extract(r"^__version__\s*=\s*version\s*=\s*['\"]([^'\"]+)", ROOT / "TradeTide" / "_version.py", "generated package version"),
    }
...
def main() -> int:
    ...
    tag = f"v{expected}"
    tag_exists = subprocess.run(["git", "rev-parse", "--verify", "--quiet", f"refs/tags/{tag}"], cwd=ROOT, check=False, stdout=subprocess.DEVNULL).returncode == 0
