...
VERSION_FILE = ROOT / "TradeTide" / "_version.py"
...
def validate_tag(tag: str) -> str:
    ...
    if match is None:
        raise ValueError("tag must use the form vMAJOR.MINOR.PATCH, for example v1.0.0")
...
def update_zenodo(version: str) -> None:
    """Update release-specific Zenodo metadata."""
...
def update_conda_recipe(version: str) -> None:
    ...
    updated_recipe, replacements = re.subn(r"(?m)^(?P<prefix>\s*version:\s*)\S+(?P<suffix>\s*(?:#.*)?)$", rf"\g<prefix>{version}\g<suffix>", recipe, count=1)
    if replacements != 1:
...
def update_pyproject(version: str) -> None:
    ...
    updated_project, replacements = re.subn(r'(?ms)(^\[project\]\n.*?^version\s*=\s*)["\'][^"\']+["\']', rf'\g<1>"{version}"', project, count=1)
    if replacements != 1:
...
def generate_version_file(version: str) -> None:
    ...
    if not VERSION_FILE.exists():
        raise RuntimeError("SCM versioning did not generate TradeTide/_version.py")
...
def create_release(tag: str, version: str) -> None:
    ...
    run("git", "add", ".zenodo.json", "conda.recipe/meta.yaml", "pyproject.toml", "TradeTide/_version.py")
...
def main() -> int:
    ...
    parser.add_argument("tag", help="annotated release tag, for example v1.0.0")
