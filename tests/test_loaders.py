...
def test_market_initialization():
    ...
    assert hasattr(market, 'load_from_database'), "Market should have load_from_database method"
...
def test_basic_database_loading(sample_market):
    ...
    assert len(sample_market.ask.dates) == len(sample_market.dates), \
        "Ask dates should match market dates"
    assert len(sample_market.bid.dates) == len(sample_market.dates), \
        "Bid dates should match market dates"

@pytest.mark.parametrize("currency_pair", MAJOR_CURRENCY_PAIRS, ids=lambda pair: f"{pair[0].value}_{pair[1].value}")
def test_load_major_currency_pairs(currency_pair):
    ...
    try:
        ...
        assert len(market.dates) > 0, f"No data loaded for {currency_pair[0].value}/{currency_pair[1].value}"
        ...
        # Verify data quality
        assert all(price > 0 for price in market.ask.open), "Ask prices should be positive"
        assert all(price > 0 for price in market.bid.open), "Bid prices should be positive"

    except Exception as e:
        pytest.skip(f"Currency pair {currency_pair} not available in database: {e}")

@pytest.mark.parametrize("currency_pair", EXOTIC_CURRENCY_PAIRS, ids=lambda pair: f"{pair[0].value}_{pair[1].value}")
...
def test_load_different_time_spans(time_span, span_id):
    ...
    try:
        ...
        if len(market.dates) > 1:
            ...
            assert actual_duration <= time_span * 1.5, \
...
def test_bid_ask_spread_validation(sample_market):
    ...
    for i in range(len(sample_market.dates)):
        ...
        assert ask_price >= bid_price, f"Ask price should be >= bid price at index {i}: ask={ask_price}, bid={bid_price}"
        ...
        assert spread_percentage < 1.0, f"Spread seems excessive ({spread_percentage:.4f}%) at index {i}"
...
def test_date_chronological_ordering(sample_market):
    ...
    for i in range(1, len(sample_market.dates)):
        assert sample_market.dates[i] > sample_market.dates[i-1], f"Dates not in chronological order at index {i}"

    # Test ask dates consistency
    for i in range(len(sample_market.ask.dates)):
        assert sample_market.ask.dates[i] == sample_market.dates[i], f"Ask date mismatch at index {i}"

    # Test bid dates consistency
    for i in range(len(sample_market.bid.dates)):
        assert sample_market.bid.dates[i] == sample_market.dates[i], f"Bid date mismatch at index {i}"
...
def test_price_data_completeness(sample_market):
    ...
    assert len(sample_market.ask.open) == len(sample_market.dates), "Ask open data incomplete"
    assert len(sample_market.ask.high) == len(sample_market.dates), "Ask high data incomplete"
    assert len(sample_market.ask.low) == len(sample_market.dates), "Ask low data incomplete"
    assert len(sample_market.ask.close) == len(sample_market.dates), "Ask close data incomplete"

    # Check bid price completeness
    assert len(sample_market.bid.open) == len(sample_market.dates), "Bid open data incomplete"
    assert len(sample_market.bid.high) == len(sample_market.dates), "Bid high data incomplete"
    assert len(sample_market.bid.low) == len(sample_market.dates), "Bid low data incomplete"
    assert len(sample_market.bid.close) == len(sample_market.dates), "Bid close data incomplete"

    # Check for NaN or invalid values
    all_ask_prices = (sample_market.ask.open + sample_market.ask.high +
                     sample_market.ask.low + sample_market.ask.close)
    all_bid_prices = (sample_market.bid.open + sample_market.bid.high +
                     sample_market.bid.low + sample_market.bid.close)

    assert all(np.isfinite(price) for price in all_ask_prices), "Ask prices contain NaN or inf"
    assert all(np.isfinite(price) for price in all_bid_prices), "Bid prices contain NaN or inf"
...
def test_invalid_currency_handling():
    ...
    try:
        ...
        pytest.warn("Same currency pair loaded without error - verify this is intended behavior")
...
def test_loading_performance():
    ...
    assert loading_time < max_time, f"Data loading took too long: {loading_time:.3f}s > {max_time}s"
...
def test_large_dataset_memory_usage(large_market):
    ...
    assert bytes_per_datapoint < 500, \
...
def test_concurrent_loading_safety():
    ...
    # Verify results
    assert len(results) + len(errors) == len(test_pairs), "Not all concurrent loading operations completed"

    # At least some should succeed (unless database unavailable)
    if len(results) == 0 and len(errors) == len(test_pairs):
        pytest.skip("All concurrent loading operations failed - database may be unavailable")
...
@pytest.mark.parametrize("currency_pair", ALL_CURRENCY_PAIRS[:5],  # Limit to first 5 for the original test
