...
class TestParseTimeStringToDelta:
    ...
    @pytest.mark.parametrize(
        "input_string,expected_type,expected_value",
        [
    ...
    @pytest.mark.parametrize(
        "input_string, expected_months, expected_years",
        [
            ("1 month", 1, 0),
            ("6 months", 6, 0),
            ("12 months", 0, 1),
    ...
    def test_relative_delta_units(self, input_string, expected_months, expected_years):
        ...
        assert (
            result.months == expected_months
        ), f"Wrong months for {input_string}, observed: {result.months}"
        assert (
            result.years == expected_years
        ), f"Wrong years for {input_string}, observed: {result.years}"
...
class TestNode:
    ...
    @pytest.mark.parametrize(
        "data", [1, "string", [1, 2, 3], {"key": "value"}, 3.14, True, False]
    )
