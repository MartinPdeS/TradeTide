...
def test_signal_initialization_with_market(market):
    ...
    assert len(signal.trade_signal) == len(
        market.dates
    ), f"Signal length ({len(signal.trade_signal)}) does not match market length ({len(market.dates)})"

    # Verify initial state
    assert (
        len(signal.trade_signal) > 0
    ), "Signal should not be empty for non-empty market"
...
def test_signal_length_matches_market(market):
    ...
    assert len(signal.trade_signal) == len(
        market.dates
    ), "Signal length does not match market"
...
def test_signal_initialization_different_markets(large_market, small_market):
    ...
    assert len(large_signal.trade_signal) > len(
        small_signal.trade_signal
...
def test_generate_random_signal_basic(market):
    ...
    assert total_signals < len(
        market.dates
    ), "Should not generate signals for every data point"
...
def test_generate_random_signal_different_probabilities(market, probability):
    ...
    assert (
        min_expected <= signal_rate <= max_expected
...
def test_generate_random_signal_reproducibility(market):
    ...
    try:
        ...
        assert (
            signal1.trade_signal == signal2.trade_signal
...
def test_signal_validation_after_random_generation(market):
    ...
    assert (
        signal.validate_against_market()
...
def test_signal_counting_accuracy(market):
    ...
    assert (
        isinstance(longs, int) and longs >= 0
    ), "Long count should be non-negative integer"
    assert (
        isinstance(shorts, int) and shorts >= 0
    ), "Short count should be non-negative integer"
...
def test_signal_with_minimal_market_data(small_market):
    ...
    assert (
        len(signal.trade_signal) > 0
    ), "Signal should have some data points even with minimal market"
...
def test_signal_memory_usage(large_market):
    ...
    assert (
        size_ratio < 100
    ), f"Signal memory usage seems excessive: {size_ratio} bytes per data point"
