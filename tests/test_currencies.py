...
class TestCurrencyListMethod:
    ...
    @pytest.mark.parametrize(
        "expected_currency", ["USD", "EUR", "GBP", "JPY", "AUD", "CAD", "CHF", "CNY"]
    )
...
class TestCurrencyPracticalUsage:
    ...
    def test_currency_filtering_and_grouping(self, all_currency_codes):
        ...
        european_currencies = [
            Currency(code) for code in european_codes if code in all_currency_codes
        ]
        asian_currencies = [
            Currency(code) for code in asian_codes if code in all_currency_codes
        ]
...
class TestCurrencyCompleteness:
    ...
    def test_all_currencies_three_letter_codes(self):
        ...
        for code in currency_list:
            ...
            assert (
                code.isalpha()
            ), f"Currency code {code} contains non-alphabetic characters"
