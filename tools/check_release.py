#!/usr/bin/env python3
"""Check TradeTide's tag-derived release metadata."""
...
def source_version() -> str:
    ...
    match = re.search(r"^__version__\s*=\s*version\s*=\s*['\"]([^'\"]+)", VERSION_FILE.read_text(encoding="utf-8"), flags=re.MULTILINE)
    if match is None:
...
def latest_tag() -> str | None:
    ...
    completed = subprocess.run(["git", "describe", "--tags", "--abbrev=0", "--match", "v[0-9]*"], cwd=ROOT, check=False, text=True, capture_output=True)
...
def main() -> int:
    ...
    tag = latest_tag()
    failures = []
    print(f"{'OK' if version == expected else 'MISMATCH':8} TradeTide/_version.py: {version}")
    if version != expected:
